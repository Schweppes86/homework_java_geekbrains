...
public class ClientPart {
    ...
    public void userInput() {
        ...
        while(true) {
            System.out.println("\nДля получения проноза погоды введите город или введите \"exit\" для выхода из программы.\n" +
                    "Если вы хотите получить прогноз погоды для Санкт-Петербурга, то введите \"SPB\"");
            city = sc.nextLine();

            if (city.equalsIgnoreCase("exit")) {break;}
            if (city.equalsIgnoreCase("SPB")) {
                com.geekbrains.part2.lesson7.ApplicationGlobalState.getInstance().setSelectedCity("Saint Petersburg");}
            else {
                com.geekbrains.part2.lesson7.ApplicationGlobalState.getInstance().setSelectedCity(city);}
